...
def fetch_batch_raw_data(from_date: Union[datetime, str], to_date: Union[datetime, str]) -> pd.DataFrame:
    ...
    # 👉 Use local data function instead
...
def load_and_process_citibike_data_from_local(year: int, months: list, base_path: str = "data/raw") -> pd.DataFrame:
    all_months_data = []
    data_dir = Path(base_path) / f"{year}-citibike-tripdata"


    for month in months:
        month_str = f"{year}{month:02d}"
        csv_files = (
        list(data_dir.rglob(f"{month_str}-citibike-tripdata*.csv")) +
        list(data_dir.rglob(f"{month_str}-citibike-tripdata*.csv.zip")) +
        list(data_dir.rglob(f"{month_str}-citibike-tripdata.zip"))
    )

        if not csv_files:
            raise FileNotFoundError(f"❌ No CSV/ZIP files found for {month_str} in {data_dir}")
